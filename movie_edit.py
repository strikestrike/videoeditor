...
Label(window, text="Audio", bg="lightgrey").grid(row=1, column=0, padx=5, pady=5, sticky=E)
...
myRecognitionButton = Button(window, textvariable=btn_search_text, command=run_downloading_thread)
btn_search_text.set("Search & Download")
myRecognitionButton.grid(row = 12, column = 0, columnspan=3, padx=10, pady=10, sticky = E)
...
def recognize_audio(audio_path):
    ...
    try:
        ...
        entry_speech.insert(-1,"Google Speech Recognition could not understand audio")
    except sr.RequestError as e:
        entry_speech.insert(-1,"Could not request results from Google Speech Recognition service; {0}".format(e))
    return speech
...
def download_video_related2audio():
    ...
    if (audio_path == ''):
      audio_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "audio_EN.mp3")
...
def output_video():
    ...
    for x in range(text_show_count):
        ...
        watermark_text = watermark_text.margin(left=100, right=100, bottom=20, opacity=0)
    ...
    watermarked_clip = CompositeVideoClip([video_clip] + text_clips, size=video_clip.size)
    ...
    final_clip = concatenate_videoclips([intro_text, watermarked_clip], padding=-1)
